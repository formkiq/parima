...
s3bucket=formkiq-distribution-core
...
for region in ${StringArray[@]}; do
   ...
   aws cloudformation package --template-file build/parima-${region}.yml --s3-bucket ${s3bucket} --profile formkiq > build/template.tmp
   ...
   aws serverlessrepo create-application \
     ...
     --template-body file://build/template.tmp --profile formkiq
done
