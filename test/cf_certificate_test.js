...
var lambda = require('../src/js/cf_certificate');
...
describe('Certificate', async()  => {
	...
	it('No Hosted Zone Found', async() => {
		let text = await readFile('./test/json/cf_certificate/create_invalid_hostedzone.json');
	...
	it('Create Certificate', async() => {
		var text = await readFile('./test/json/cf_certificate/create.json');
		...
		text = await readFile('./test/json/cf_certificate/delete.json');
	...
	it('Delete Certificate failed', async() => {
		var text = await readFile('./test/json/cf_certificate/delete.json');
		var response = await lambda.handler(JSON.parse(text), {logStreamName:"test"});
		assert.equal(200, response[1]);
	...
	it('Update Change Domains', async() => {
		var text = await readFile('./test/json/cf_certificate/update_certificate_change.json');
		var response = await lambda.handler(JSON.parse(text), {logStreamName:"test"});
		assert.equal(200, response[1]);
	...
	it('Update Certificate', async() => {
		var text = await readFile('./test/json/cf_certificate/update.json');
		var response = await lambda.handler(JSON.parse(text), {logStreamName:"test"});
		assert.equal(200, response[1]);
	...
	it('Invalid RequestType', async() => {
		var text = await readFile('./test/json/cf_certificate/invalid.json');
		var response = await lambda.handler(JSON.parse(text), {logStreamName:"test"});
		assert.equal(200, response[1]);
	...
	it('Missing RequestType', async() => {
		var text = await readFile('./test/json/cf_certificate/missing_requesttype.json');
		var response = await lambda.handler(JSON.parse(text), {logStreamName:"test"});
